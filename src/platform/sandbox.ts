...
const sandboxBookId='00000000-0000-4000-8000-000000000202';
...
async function fundMaker(db:Database,accountId:string){
  const effectId=`sandbox-liquidity:${accountId}`;
  await db.transaction(async sql=>{
    ...
    const available=await ledgerAccount(sql,accountId,'NGN','user_available');
    await postJournal(sql,{effectId,asset:'NGN',kind:'financial_correction',referenceId:accountId,
      reason:'Hosted sandbox liquidity only',lines:[{account:escrow,debit:5_000_000n,credit:0n},
        {account:available,debit:0n,credit:5_000_000n}]});
  });
}

async function seedOrder(db:Database,actor:Account,outcome:string,side:'buy'|'sell',price:string){
  await db.transaction(async sql=>{
    ...
    const exists=(await sql.query(`SELECT 1 FROM clob_orders WHERE book_id=$1 AND owner_id=$2 AND outcome_id=$3
      AND side=$4 AND state='open' LIMIT 1`,[sandboxBookId,actor.id,outcome,side])).rows.length;
    if(exists)return;
    await submitOrder(sql,actor,sandboxMarketId,{asset_code:'NGN',outcome_id:outcome,side,
      limit_price:price,quantity:'100'},`sandbox-liquidity:${outcome}:${side}`);
  });
}
...
export async function bootstrapSandbox(db:Database){
  const actors=await db.transaction(async sql=>{
    ...
    await sql.query(`UPDATE financial_assets SET synthetic=true,approved=true,evidence_ref='sandbox:swervpay-development'
      WHERE code='NGN'`);
    ...
    await sql.query(`INSERT INTO clob_asset_bindings(policy_ref,asset_code,contract_unit_minor,approved,evidence_ref)
      VALUES ('sandbox:ngn-collateral-v1','NGN',10000,true,'sandbox:test-market') ON CONFLICT DO NOTHING`);
    await sql.query(`INSERT INTO clob_markets(id,market_id,asset_code,contract_unit_minor,status,activated_by)
      VALUES ($1,$2,'NGN',10000,'open',$3) ON CONFLICT (market_id,asset_code) DO NOTHING`,
    [sandboxBookId,sandboxMarketId,sandboxCreatorId]);
    return (await sql.query<Account>(`SELECT * FROM accounts WHERE id=ANY($1::uuid[]) ORDER BY id`,
      [[sandboxBidderId,sandboxSellerId]])).rows;
  });
  await fundMaker(db,sandboxBidderId);await fundMaker(db,sandboxSellerId);
  ...
  for(const outcome of ['yes','no']){
    await seedOrder(db,bidder,outcome,'buy','450000');
    await seedOrder(db,seller,outcome,'sell','550000');
  }
}
