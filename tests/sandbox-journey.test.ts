...
let db:Database,app:FastifyInstance,token:string;
...
beforeAll(async()=>{
  ...
  expect(registered.statusCode,registered.body).toBe(201);token=registered.json<{access_token:string}>().access_token;
});
...
describe('hosted NGN sandbox journey',()=>{
  it('restarts without duplicating liquidity funding or resting orders',async()=>{
    ...
    expect(funding.count).toBe('2');expect(orders.count).toBe('4');
  });
